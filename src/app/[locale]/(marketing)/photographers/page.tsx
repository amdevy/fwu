        ...
        {photographers.map((p, i) => {
          ...
          return (
            <Link
              ...
              className="photog-list-row"
            >
              <div style={{ width: 60, flexShrink: 0, fontFamily: 'var(--font-mono)', fontSize: 12, letterSpacing: '0.14em', color: 'var(--fg-muted)' }}>
                N°{String(i + 1).padStart(2, '0')}
              </div>
              <div className="display" style={{ fontSize: 'clamp(28px, 4vw, 48px)', lineHeight: 1.05 }}>
                {p.name[locale]}
              </div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, letterSpacing: '0.06em', color: 'var(--fg-muted)' }}>
                {designerCount} {ua ? 'дизайнерів' : 'designers'}
                {generalCount > 0 && ` · ${generalCount} ${ua ? 'загальних' : 'general'}`}
              </div>
              <div style={{ marginLeft: 'auto', fontFamily: 'var(--font-mono)', fontSize: 12, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--fg-muted)' }}>
                {ua ? 'Відкрити →' : 'Open →'}
              </div>
            </Link>
          )
